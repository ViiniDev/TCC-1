...
// Gera matriz NxN com distribuição escolhida
double** gera_matriz(int N, dist_t tipo) {
    ...
    for (int i = 0; i < N; i++) {
        ...
        for (int j = 0; j < N; j++) {
            switch (tipo) {
                case UNIFORME: mat[i][j] = rand_uniform(); break;
                case NORMAL:   mat[i][j] = rand_normal();  break;
                case EXPONENCIAL: mat[i][j] = rand_exponencial(1.0); break;
            }
        }
    }
    return mat;
...
double** gera_kernel(int K) {
    ...
    // Normaliza kernel (para não explodir valores)
    for (int i = 0; i < K; i++) {
        for (int j = 0; j < K; j++) {
            ker[i][j] /= soma;
        }
...
// Convolução 2D simples (sem padding)
